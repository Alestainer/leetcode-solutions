...
class Solution:
    ...
    def find_a_path(self, start, end):
        ...
        if (end in self.dictionary[start]):
            ...
            return -1
        elif max(map(lambda x: Solution().find_a_path(start=x, end=end),
                     list(self.dictionary[start] - self.visited))) == -1:
            return -1
        else:
...
if (__name__ == 'main.py'):
    ...
    sys.stdout.write(Solution(dictionary=dictionary).find_a_path(start=start, end=end)
                     if (
    Solution(dictionary=dictionary).find_a_path(start=start, end=end) != -1) else 'route not found')
